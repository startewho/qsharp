...
namespace QuickJs
{
    ...
    public struct JSAtom
    {
        ...
        /// </returns>
        public unsafe string ToString(JSContext* ctx)
        {
            if (_value == 0)
                return null;

            byte* str = JS_AtomToCString(ctx, (uint)_value);
            try
            {
                return Utils.PtrToStringUTF8(new IntPtr(str));
            }
            finally
            {
        ...
        public unsafe JSValue ToStringValue(JSContext* ctx)
        {
            return JS_AtomToString(ctx, (uint)_value);
        }
        ...
        /// </returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe JSValue ToValue(JSContext* ctx)
        {
            return JS_AtomToString(ctx, (uint)_value);
        }
